...
insert into bank_customer values ("941127-1000006", "이순신", 1, "010-1234-1006", "부산시 영동구");
...
insert into bank_account values ("101-13-1005", "S3", "주택청약예금", "841127-1000006", 720800, "2012-10-15");
...
# 실습 2-14
select * from bank_transaction where t_dist = 2 order by t_amount desc limit 3; 

# 실습 2-15
select sum(t_amount) as 입금총합,
	   avg(t_amount) as 입급평균
       from bank_transaction
       where t_dist = 1;

# 실습 2-16
select max(t_amount) as 큰금액,
	   min(t_amount) as 작은금액
       from bank_transaction
       where t_dist = 2;
